...
{#if (isMobile)}
    <nav class={`fixed -translate-y-[0.2rem] top-2 left-1/2 -translate-x-1/2 flex flex-col w-[90%] bg-neutral-950/80 backdrop-blur-sm px-2 pt-2 pb-3 rounded-3xl`}>
...
{:else}
    <nav class={`fixed h-[60px] top-2 left-1/2 -translate-x-1/2 flex justify-between items-center gap-4 bg-neutral-950/95 px-4 rounded-3xl`}>
        ...
        <div class={`h-[40px] flex gap-2 ubuntu-light text-sm`}>
            <a class={`h-full px-6 rounded-full grid place-items-center backdrop-blur-sm border-2 ${$page.url.pathname === '/profile' ? 'border-neutral-400' : 'border-neutral-800 hover:border-neutral-600'}`} href="/profile">Profile</a>
            <a class={`h-full px-6 rounded-full grid place-items-center backdrop-blur-sm border-2 ${$page.url.pathname === '/leaderboards' ? 'border-neutral-400' : 'border-neutral-800 hover:border-neutral-600'}`} href="/leaderboards">Leaderboards</a>
            <a class={`h-full px-6 rounded-full grid place-items-center backdrop-blur-sm border-2 ${$page.url.pathname === '/history' ? 'border-neutral-400' : 'border-neutral-800 hover:border-neutral-600'}`} href="/history">History</a>
            <a class={`h-full px-6 rounded-full grid place-items-center backdrop-blur-sm border-2 ${$page.url.pathname === '/about' ? 'border-neutral-400' : 'border-neutral-800 hover:border-neutral-600'}`} href="/about">About</a>
        </div>
    </nav>
{/if}